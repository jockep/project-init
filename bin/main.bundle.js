      ...
      Object.keys(protocols).forEach(function(scheme) {
        ...
        function request(input2, options, callback) {
          if (isString2(input2)) {
            var parsed;
            try {
              parsed = urlToOptions(new URL2(input2));
            } catch (err) {
            ...
            input2 = parsed;
          } else if (URL2 && input2 instanceof URL2) {
...
var cliInterface = (0, import_promises.createInterface)({ input: import_node_process.stdin, output: import_node_process.stdout });
import_node_console.default.read = (msg) => cliInterface.question(msg);
import_node_console.default.close = () => cliInterface.close();
async function setupProjectFiles() {
  await (0, import_promises2.mkdir)("src");
  await (0, import_promises2.writeFile)("src/main.ts", "", "utf-8");
}
async function createTSConfig() {
  ...
  await (0, import_promises2.writeFile)("tsconfig.json", JSON.stringify(tsConfig, null, "	"));
}
async function createPackageJson() {
  ...
  const packageJson = {};
  const scripts = {};
  packageJson.name = await import_node_console.default.read("Name: ");
  packageJson.description = await import_node_console.default.read("Description: ");
  packageJson.version = await import_node_console.default.read("Version: ");
  packageJson.author = await import_node_console.default.read("Author: ");
  packageJson.license = await import_node_console.default.read("License: ");
  ...
  const bundle = await import_node_console.default.read("Setup bundler? [Y/n]");
  ...
  const deps = await import_node_console.default.read("Add dependencies separated by space eg. <typescript axios>: ");
  if (deps.length > 0) {
  ...
  const devDeps = await import_node_console.default.read("Add devDependencies separated by space eg. <react svelte>: ");
  if (deps.length > 0) {
  ...
  await (0, import_promises2.writeFile)("package.json", JSON.stringify(packageJson, null, "	"));
}
...
async function addNPMPackage(name, dependencyObject) {
  try {
    const request = await axios_default.get(`https://registry.npmjs.org/${name}`);
    const data = request.data;
    dependencyObject[data.name] = `^${data["dist-tags"].latest}`;
    import_node_console.default.log(import_chalk.default.green(`${data.name} found adding version: ${data["dist-tags"].latest} to package.json...`));
  } catch (e) {
    import_node_console.default.log(import_chalk.default.red(`${name} was not found.`));
  }
}
async function main() {
  await createPackageJson();
  import_node_console.default.log(import_chalk.default.blue("package.json created..."));
  await createTSConfig();
  import_node_console.default.log(import_chalk.default.blue("tsconfig.json created..."));
  await setupProjectFiles();
  import_node_console.default.log(import_chalk.default.blue("Project structure created..."));
  import_node_console.default.log(import_chalk.default.green("completed, run yarn!"));
  import_node_console.default.close();
}
main();
